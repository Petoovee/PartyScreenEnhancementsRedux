...
{
    /// <summary>
    ///     This is the base class for all Sorters, it takes care of verifying that the to be compared units aren't
    ///     heroes/companions.
    ///     The entire sorting structure is essentially a linked list, where the next link is used when a sorter encounters
    ///     equality
    /// </summary>
    ...
    public abstract class PartySort : IComparer<PartyCharacterVM>
    {
        protected PartySort(PartySort equalSorter, bool descending, List<string> customSort)
        {
            ...
            if (customSort != null) CustomSettingsList = customSort;
        }
        ...
        [XmlElement("Descending")] public bool Descending { get; set; }
        ...
        public int Compare(PartyCharacterVM x, PartyCharacterVM y)
        {
            ...
            if (x.Character.IsPlayerCharacter) return -1;
            if (y.Character.IsPlayerCharacter) return 1;
            if (x.IsHero && !y.IsHero) return PartyScreenConfig.ExtraSettings.KeepHeroesOnTop ? -1 : 1;
            if (y.IsHero && !x.IsHero) return PartyScreenConfig.ExtraSettings.KeepHeroesOnTop ? 1 : -1;
            if (x.IsHero && y.IsHero) return 0;
